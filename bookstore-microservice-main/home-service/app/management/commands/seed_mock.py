...
from app.models.book import BookStatus, ProductType
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Chỉ seed sản phẩm với loại cụ thể (vd: book, electronics, audio). Mặc định seed tất cả.",
        )
    ...
    def handle(self, *args, **options):
        ...
        product_types_dict = {p.value: p.label for p in all_product_types}

        # Nếu --product-type được truyền, chỉ seed loại này
        filter_product_type = options.get("product_type")
        if filter_product_type:
            # Tìm ProductType enum matching với giá trị
            matching_types = [p for p in all_product_types if p.value == filter_product_type]
            if not matching_types:
                self.stdout.write(
                    self.style.ERROR(f"❌ Loại sản phẩm '{filter_product_type}' không tồn tại. "
                    f"Các loại hợp lệ: {', '.join([p.value for p in all_product_types])}")
                )
                return
            product_types = matching_types
            self.stdout.write(self.style.SUCCESS(f"📌 Chỉ seed loại sản phẩm: {filter_product_type}"))
        else:
        ...
        for i in range(total_books):
            ...
            mo_ta = (
                f"Sản phẩm chia sẻ về {chu_de[i % len(chu_de)]}, "
                f"được thiết kế với chất lượng cao và giá trị thực tế."
            )
            ...
            books_data.append({
                "title": f"{product_types_dict[product_type.value]} - {tieu_de} (#{i + 1})",
                "isbn": f"978604{2000000 + i}" if product_type == ProductType.BOOK else "",
                "description": mo_ta,
                "publication_year": 2016 + (i % 10),
                "page_count": 160 + (i % 14) * 16 if product_type == ProductType.BOOK else None,
