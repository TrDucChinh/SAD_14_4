...
class BookListCreateView(APIView):
    def get(self, request):
        """Khách hàng và mọi người đều được xem danh sách sách và sản phẩm khác."""
        search = None
        book_status = request.query_params.get("status")
        product_type = request.query_params.get("product_type")  # Filter by product type
        books = _svc.list_books(search=search, status=book_status)
        # Filter by product_type if specified
        if product_type:
            books = [b for b in books if getattr(b, 'product_type', 'book') == product_type]
